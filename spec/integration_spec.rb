...
describe 'AWS IAM Integration' do
  ...
  describe generic_policy_source("arn:aws:iam::#{get_account_id}:user/some_user_with_admin_permissions") do
    it {
      should be_allowed_to('sts:AssumeRole')
          .with_resource("arn:aws:iam::#{get_account_id}:role/Administrator")
    }
    it {
      should_not be_allowed_to('sts:AssumeRole')
          .with_resource("arn:aws:iam::#{get_account_id}:role/SomeOtherRole")
    }
  end
